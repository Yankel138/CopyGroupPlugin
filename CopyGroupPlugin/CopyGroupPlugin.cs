    ...
    public class CopyGroup : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                Room pickedRoom = GetRoomByPoint(doc, point);
                XYZ pickedRoomCenter = GetElementCenter(pickedRoom);
                XYZ pointPickedRoom = pickedRoomCenter + offset;

                Transaction ts = new Transaction(doc);

                ts.Start("Копирование группы объектов");
                doc.Create.PlaceGroup(pointPickedRoom, group.GroupType);
                ts.Commit();
            }
